    ...
    (async () => {
      ...
      try {
        const api = await fetch(
          "/api/announce?" +
            new URLSearchParams({ name: cData[index].name }).toString()
        );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row">
        ...
        >
          {width <= 640 && (
            <span className="px-6 py-3 text-sm font-light">
              เลือกประกาศที่ต้องการ
            </span>
          )}
          {announce &&
            announce.map((a, i) => (
          ...
          {data[index] && data[index].success !== undefined ? (
            <>
              ...
              {data[index].success === false ? (
                <h4 className="py-3 text-lg font-bold text-red-500">
